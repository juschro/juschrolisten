...
function loadConfig() {
  ...
  if (!DISCORD_TOK) throw "invalid or missing DISCORD_TOK";
...
if (SPEECH_METHOD === "vosk") {
  ...
  recs = {
    en: new vosk.Recognizer({
      model: new vosk.Model("vosk_models/en"),
      sampleRate: 48000,
    }),
    de: new vosk.Recognizer({
      model: new vosk.Model("vosk_models/de"),
      sampleRate: 48000,
    }),
  ...
  voice_Connection.on("speaking", async (user, speaking) => {
    ...
    audioStream.on("end", async () => {
      ...
      try {
        ...
        if (out != null) process_commands_query(out, mapKey, user);
      } catch (e) {
